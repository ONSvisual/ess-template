...
import InfoButton from "$lib/bespoke_components/other/InfoButton.svelte";
...
const onMouseenterEvent = () => { expandIcon = true };

const onMouseleaveEvent = () => { expandIcon = false };

const onClickEvent = () => { displayAdditional = !displayAdditional };
...
>
    <span style="font-weight: bold">{titleText}<span style="font-weight: normal">{unitDescriptionText === "" ? "" : ", "+unitDescriptionText}</span><InfoButton {expandIcon}></InfoButton></span>
