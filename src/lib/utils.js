...
import { cdnUrl, geoCodesLookup, geoNames, addTheArray, areaPluralObject } from "$lib/config";
...
export const addTheToName = (name) => {

  return (addTheArray.includes(name) ? "the " : "") + name;
}

export const constructSameRegionAreasLabel = (geogLevel, parentName) => {

  let areaPlural = areaPluralObject[geogLevel];

  return ["other", "country"].includes(geogLevel) ? undefined : "Other "+areaPlural+" in "+addTheToName(parentName);;
} 
